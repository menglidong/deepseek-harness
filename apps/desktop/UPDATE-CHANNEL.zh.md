...
**CDN 刷新：不做**（用户决策 2026-09-22，脚本只上传）。**必须**在七牛控制台「CDN → 域名 → 缓存配置/HTTP 响应头规则」给 `/dsh-desk/feeds/*` 与 `/api/v0/*` 配 `Cache-Control: no-store`（一次性配置，脚本零参与；这是缓存规则，不是刷新）。原因（2026-09-23 实测）：清单/策略是**同 key 覆盖**上传，而默认边缘缓存 TTL ≥ 9 小时——不配该规则时，发新版清单后各机器应用内检查更新会长时间看到旧版本，更新通道形同虚设。exe 的 key 每版本唯一、不被覆盖，无需此规则。2026-09-23 用户已在控制台配置：`/api/v0`、`/dsh-desk/feeds` 两目录 10 秒 TTL（等效 no-store），实测保存后立即回源生效；同次验证发现 exe 同 key 覆盖仍受全局 1 个月 TTL 保护（旧 exe 留在边缘属预期，新发版用新 key 不受影响）。
